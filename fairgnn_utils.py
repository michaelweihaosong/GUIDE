...
#####################  pokec_n ########################
def load_pokec(dataset, sens_attr, predict_attr, path="../dataset/pokec/", label_number=1000, sens_number=1000):
    """Load data"""
...
#####################  compas  ########################
def load_compas(dataset, sens_attr="race", predict_attr="is_recid", path="./dataset/compas/", label_number=1000, sens_number=1000):
    print('Loading {} dataset from {}'.format(dataset, path))
    idx_features_labels = pd.read_csv(os.path.join(path,"{}.csv".format(dataset)))
    header = list(idx_features_labels.columns)
    header.remove(predict_attr)
...
#####################  income  ########################
def load_income(dataset, sens_attr="race", predict_attr="income", path="./dataset/income/", label_number=1000, sens_number=1000):
    print('Loading {} dataset from {}'.format(dataset, path))
    idx_features_labels = pd.read_csv(os.path.join(path,"{}.csv".format(dataset)))
    header = list(idx_features_labels.columns)
    header.remove(predict_attr)
